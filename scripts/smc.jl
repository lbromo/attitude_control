...
include("../src/c_smc_controller.jl")
...
function smc_cb(int)
    ...
    τ_mag = cross(u_mag, get_B_field_body(int) * 1e-9)
    p.u_mag = τ_mag
end
...
L = [0.5 0.8660 0; 0.5 0 0.8660;  0.5 -0.8660 0; 0.5 0 -0.8660]
orbit = init_orbit_propagator(Val(:J4), tle);

set_inertia(J)
set_wheel_matrix(L)
set_default_ctrl_conf()

pars = Parameters(J=J, L=pinv(L), orbit=orbit)

# Random initial conditions
q₀ = randn(4); q₀ = q₀ / norm(q₀);
ω₀ = randn(3); ω₀ = ω₀ / norm(ω₀) * deg2rad(0);
x₀ = [q₀; ω₀]
...
saved_values = SavedValues(Float64, default_save_func_types)
callbacks = CallbackSet(
    PeriodicCallback(smc_cb, pars.Δt),
    SavingCallback(default_save_func, saved_values),
)
...
u_body = map(v -> pinv(L)*v, eachcol(u))
u_body = reduce(hcat, u_body)
h_body = map(v -> pinv(L)*v, eachcol(h)) # Get the u vector
h_body = reduce(hcat, h_body)
